...
int sc_current = -1,sc_tail=-1,sc_flag=0;
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	i=0;
	while(i<4)
	{			/* to map 16MB XINU memory we require 4page tables, populate first 4 page tables  */
		get_frm(&frame_number);
		pd_pointer = ((i+FRAME0)*NBPG);
		
		frm_tab[frame_number].fr_status = FRM_MAPPED;
		frm_tab[frame_number].fr_pid[currpid] = 0;	/*  initialize for NULL process */
		frm_tab[frame_number].fr_type = FR_TBL;
		
	    k=0;
		while(k<1024)
		{			/* insert 1024 entries for page tables  */
			pd_pointer->pt_pres = 1;
			pd_pointer->pt_write = 1;
			pd_pointer->pt_user=1;
			pd_pointer->pt_pwt = 1;
			pd_pointer->pt_pcd = 0;
			pd_pointer->pt_acc = 0;
			pd_pointer->pt_dirty = 0;
			pd_pointer->pt_mbz = 0;
			pd_pointer->pt_global = 1;
			pd_pointer->pt_avail = 0;
			pd_pointer->pt_base = ((i*FRAME0)+k);
			pd_pointer++;
			k++;
	...
	setPageDirectory(NULLPROC);		/* create page directory */
